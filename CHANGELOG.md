...
- `/export-story` 先写 `书名.草稿.md`，再抄一份 `书名.md`。叙事者按这份本地草稿二次润色，只覆盖成稿。
- 文学会话没有读盘工具，草稿全文贴进润色提示，禁止再去调 bash。

## 0.4.11 — 2026-08-18

- `/export-story` 先落下可读草稿，再请叙事者润色排版，完成后覆盖同一份。
- 润色提示里写死日期，并禁止调工具。若模型仍吐出 `runshell`，保留草稿。

## 0.4.10 — 2026-08-18

- `/export-story` 当场把已抽出的正文装订成 Markdown，不再叫醒模型去「润色」。
- 模型不会再为了写日期去调 `runshell`，也不会把工具调用当成书稿。

## 0.4.9 — 2026-08-18

- `/export-story` 只取助手消息里的正文块，不再把英文 `reasoning` 构思拼进素材。
- 构思里若提前写了「【歧路】」，不再把后面整段小说裁掉。旧会话可以直接誊。
