...
import { defaultMeta, defaultProtagonist, formatStoryMeta, parseLoreEntry, parseStoryMeta, } from 'infinite-core';
...
export function metaPath(root) {
    return join(root, META_FILE);
}
export function hasStory(root) {
    try {
...
export function saveMeta(root, meta) {
...
export function loadWorldbook(root) {
    return readMarkdownEntries(join(root, WORLD_DIR)).filter((entry) => entry.category !== '写法' && entry.category !== '剧情');
}
export function loadCharacters(root) {
    return readMarkdownEntries(join(root, CHAR_DIR));
}
export function loadPlots(root) {
    return readMarkdownEntries(join(root, PLOT_DIR));
}
export function listTemplatePlots(config, templateId) {
    return loadPlots(templatePath(config, templateId));
}
export function listTemplateCharacters(config, templateId) {
    return loadCharacters(templatePath(config, templateId));
}
export function applyOpening(root, plot) {
    mkdirSync(join(root, WORLD_DIR), { recursive: true });
    writeFileSync(join(root, WORLD_DIR, 'opening.md'), [
        '---',
        'id: opening',
        `title: ${plot.title}`,
        'category: 开篇',
...
export function saveArchive(root, text) {
    if (!text)
        return;
    mkdirSync(root, { recursive: true });
...
export function applyProtagonistIdentity(root, name) {
...
export function templatePath(config, templateId) {
    return join(templatesDir(config), templateId);
}
export function seedStory(root, templateId, config, force) {
...
export function writeProtagonistCard(root, name) {
    mkdirSync(join(root, CHAR_DIR), { recursive: true });
