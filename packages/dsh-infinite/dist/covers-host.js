...
export function coversRoot(config) {
    return join(dirname(templatesDir(config)), 'covers');
}
export function safeCoverName(name) {
...
function resolveUnder(root, name) {
    ...
    if (!candidate.startsWith(prefix) && candidate !== normalize(root).toLowerCase())
        return null;
