import { copyFileSync, existsSync, readdirSync, readFileSync, renameSync, unlinkSync, writeFileSync, } from 'node:fs';
...
export function repairLegacyBindEvents(config) {
    return repairSessionTree(join(resolveDshHome(config), 'sessions'));
}
export function repairSessionTree(root) {
    const files = [];
    const repaired = [];
    let failed = 0;
...
export function repairSessionLog(path) {
...
export function patchJsonl(text) {
