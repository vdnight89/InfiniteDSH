export function isNoAskProvider(error) {
...
export function canAsk(ctx) {
    const questions = ctx.userQuestions;
    if (typeof questions?.ask !== 'function')
        return false;
    if (questions.hasProvider === false)
        return false;
    return true;
}
export function stripRecommend(label) {
    return label.replace(/\s*(?:\((?:recommended|推荐)\)|（(?:recommended|推荐)）)\s*$/i, '').trim();
}
export function pickAnswer(answers, id) {
...
export async function askUser(ctx, inv, questions) {
