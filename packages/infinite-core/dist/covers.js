...
export function coverFileForLabel(label) {
    const trimmed = label.replace(/\s*(?:\((?:recommended|推荐)\)|（(?:recommended|推荐)）)\s*$/i, '').trim();
    if (CHARACTER_COVERS[trimmed])
        return CHARACTER_COVERS[trimmed];
    const lower = trimmed.toLowerCase();
    for (const item of TEMPLATE_CATALOG) {
        if (item.label === trimmed || item.id === lower)
            return `${item.id}.jpg`;
...
export function buildCoverManifest() {
