...
export function stripTrailingFork(text: string): string {
...
export function isPlanningParagraph(text: string): boolean {
  const t = text.trim()
  if (!t) return true
  if (PLANNING_LINE.test(t) || PLANNING_CN_LINE.test(t)) return true
  for (const marker of PLANNING_MARKERS) {
    if (marker.test(t)) return true
  }
...
export function isPlanningDump(text: string): boolean {
  const t = text.trim()
  if (!t) return false
  if (/^(?:We need|Need obey|The user |用户让我|我们需要回应|我要推进|让我构思|我写正文)/i.test(t)) return true
  const paras = t.split(/\n\s*\n/)
...
export function extractStoryBody(text: string): string {
  ...
  for (let i = runs.length - 1; i >= 0; i -= 1) {
    ...
    if (cjk >= 24 && !isPlanningDump(body)) return body
  }
  if (isPlanningDump(cleaned)) return ''
  return cleaned
}

/** Strip 歧路 and planning, but keep Markdown headings for a polished book. */
export function cleanManuscript(text: string): string {
...
export function manuscriptHasBody(text: string): boolean {
...
export function bindManuscript(title: string, protagonist: string, world: string, source: string): string {
  const body = extractStoryBody(source) || source.trim()
  if (countCjk(body) < 24) return ''
  return [
    `# ${title}`,
    '',
