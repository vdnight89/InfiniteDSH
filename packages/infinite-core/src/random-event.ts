...
export function pickRandomEventEntry(
  ...
  const pool = entries.filter(
    (e) => !e.disabled && !e.constant && !matchedIds.has(e.id) && !excluded.has(e.id),
  )
...
export function formatRandomEvent(entry: LoreEntry): string {
  return (
    `【随机世界事件】以下设定来自当前规则书，可作为本段剧情的新进展、转折或悬念自然引出` +
